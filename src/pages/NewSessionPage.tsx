...
export default function NewSessionPage() {
  ...
  const [rateType, setRateType] = useState<'standard' | 'premium' | 'custom' | 'consumption'>('standard')
      ...
      <main className="pt-20 px-4 space-y-8">
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <button
              onClick={() => setRateType('consumption')}
              className={`w-full p-4 rounded-xl border flex items-center justify-between transition-all ${
                rateType === 'consumption' ? 'bg-accent-glow border-accent' : 'bg-surface border-border'
              }`}
            >
              <div className="flex items-center gap-3">
                <Zap size={18} className={rateType === 'consumption' ? 'text-accent' : 'text-text3'} />
                <div className="text-left">
                  <div className="text-sm font-bold text-text">Consommation</div>
                  <div className="text-xs text-text2">Uniquement les produits consommés (0 DH/h)</div>
                </div>
              </div>
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                rateType === 'consumption' ? 'bg-accent border-accent' : 'border-border'
              }`}>
                {rateType === 'consumption' && <div className="w-2 h-2 bg-white rounded-full" />}
              </div>
            </button>
          </div>
        </section>
